...
export function ExportDataModal({ isOpen, onClose }: ExportDataModalProps) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      showToast(t("profile.dataExported"));
      onClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t("profile.exportTitle")} size="sm">
      <div className="flex flex-col items-center gap-4 py-2">
        ...
        <Button fullWidth onClick={handleExport} disabled={isExporting}>
          {isExporting ? t("profile.exporting") : t("profile.exportButton")}
        </Button>
      </div>
    </Modal>
  );
