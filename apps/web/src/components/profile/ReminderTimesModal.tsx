...
export function ReminderTimesModal({
  ...
  async function handleSave() {
    ...
    try {
      ...
      showToast(t("reminderTimesModal.updated"));
      onSaved(times);
      onClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t("reminderTimesModal.title")} size="sm">
      <div className="flex flex-col gap-3">
        {times.length === 0 && (
          <p className="text-sm text-neutral-500 text-center py-2">
            {t("reminderTimesModal.noReminders")}
          </p>
        )}
        {times.map((time, i) => (
        ...
        {times.length < 5 && (
          ...
          >
            ...
            {t("reminderTimesModal.addReminder")}
          </button>
        )}
      </div>
      <Button
      ...
      >
        {isSubmitting ? t("reminderTimesModal.saving") : t("reminderTimesModal.save")}
      </Button>
    </Modal>
  );
}
