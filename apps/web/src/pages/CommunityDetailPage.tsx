...
export function CommunityDetailPage() {
  ...
  async function handleAddToLibrary() {
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === "ALREADY_PURCHASED") {
        showToast(t("community:alreadyInLibraryToast"), "info");
      } else {
  ...
  async function handleReviewDeleted(reviewId: string) {
    ...
    try {
      ...
      showToast(t("community:reviewDeleted"));
    } catch (err) {
  ...
  return (
    <div className="pb-4">
        ...
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-sm text-neutral-500">
              {t("community:byCreator", { name: detail.creator.displayName })}
            </p>
          </div>
          {!detail.isOwner && (
            <DropdownMenu
          ...
          <span className="flex-shrink-0 rounded-full bg-success-100 px-3 py-1 text-sm font-bold text-success-700">
            {t("free")}
          </span>
        </div>
      ...
      <div className="mx-5 mt-6">
        ...
        {(detail.isPurchased || true) && !detail.isOwner && (
          <div className="mb-4">
            <ReviewForm
              deckId={deckId!}
